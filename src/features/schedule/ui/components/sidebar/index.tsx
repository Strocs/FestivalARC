import { useState, useEffect } from 'react'
import type { UIHeaderItem } from '../../types'
import { cn } from '@/features/shared/utils'
import { CollapseButton } from './CollapseButton'
import { NavigationButtons } from './NavigationButtons'
import { DaySelector } from './DaySelector'
import { StageFilter } from './StageFilter'
import { StageSelector } from './StageSelector'

interface SidebarProps {
  stages: ReadonlyArray<UIHeaderItem>
}

const STORAGE_KEY = 'festivalarc-sidebar-expanded'

export function Sidebar({ stages }: SidebarProps) {
  // Expanded State
  // TODO: create a specific hook to manage localStorage across differents keys
  const [isExpanded, setIsExpanded] = useState(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem(STORAGE_KEY)
      return stored ? JSON.parse(stored) : true
    }
    return true
  })

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(isExpanded))
    }
  }, [isExpanded])

  return (
    <aside
      className={cn(
        'sticky top-[84dvh] z-10 flex h-fit w-full shrink-0 flex-col gap-2 px-3 transition-[width_top] duration-300 ease-in-out md:top-4 md:px-4',
        isExpanded ? 'md:w-72' : 'md:w-30',
      )}>
      <NavigationButtons className='hidden items-center justify-between md:flex' />
      <DaySelector isExpanded={isExpanded} />
      <StageSelector stages={stages} />
      <StageFilter stages={stages} isExpanded={isExpanded} />
      <div className='bg-25-black hidden rounded-xs px-4 py-2 md:block'>
        <CollapseButton
          isExpanded={isExpanded}
          onToggle={() => setIsExpanded(!isExpanded)}
        />
      </div>
    </aside>
  )
}
